    ...
    if (inMoves[searchGame]){
        ...
        for (const [key, value] of Object.entries(foundGame)){
            ...
            inMoves[searchGame][key].forEach((rawMove) => {
                if (key === 'level-up'){
                    const found = moves.find((dexMove) => dexMove.name.english === rawMove.name)
                    if (found) {
                    ...
                    const found = moves.find((dexMove) => dexMove.name.english === rawMove)
                    if (found) {
                        pokemonMoves[key].push({
                            name: found.name.english,
                            type: found.type,
...
const readPokemonByGame = asyncHandler(async (request, response, next) => {
    ...
    if (game === 'isle-of-armor' || game === 'crown-tundra' || game === 'swsh-other') { mainGame = 'sword-shield'}
    const newMoves = mainGame ? getPokemonMoves(pokemon.moves, mainGame, moves) : getPokemonMoves(pokemon.moves, game, moves); 
...
const readPokemon = asyncHandler(async (request, response) => {
    const { pokemon, moves } = response.locals, game = 'scarlet-violet';
