...
export default function PokemonPage({
  ...
  // This is for Form Chnages, in the future we can limit it if there is no form change.
  const pokemonInitial = () => pokemon;
  const gameInitial = () => pokemon.gameDropDown[0];
  const movesInitial = () => pokemon.moves[pokemon.gameDropDown[0].key];
  ...
  const changeSelectedGame = (game) => {
    ...
    setSelectedMoves(selectedPokemon.moves[game.key]);
  };
  ...
  return (
    ...
    >
      ...
      <div className={"col-flex space-y-2"}>
        <PokemonIndToolbar id={pokemon._id} game={game} />
