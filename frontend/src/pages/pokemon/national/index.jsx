...
const NationalDexRow = ({ pokemon: { _id, baseStats: { hp, atk, def, spatk, spdef, spd }, name, type, abilities } }) => {
    ...
    return (
        <tr className="text-center hover:bg-purple-200 hover:text-gray-900 hover:font-bold" >
            <td>{_id}</td>
            <td><Link href={`/pokemon/national/${_id}/sword-shield`} passhref><a>{name.english}</a></Link></td>
            <td>
                <Image
                    src={`/sprites/${_id}.png`}
                    alt={`${name.english}`}
...
mapDispatchToProps = (dispatch) => {
    return {
        loadNationalDex: bindActionCreators(pokemonActions.loadNationalDex, dispatch)
    };
}
